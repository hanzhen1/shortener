...
var (
	...
	cacheMallShortUrlMapIdPrefix   = "cache:mall:shortUrlMap:id:"
	cacheMallShortUrlMapMd5Prefix  = "cache:mall:shortUrlMap:md5:"
	cacheMallShortUrlMapSurlPrefix = "cache:mall:shortUrlMap:surl:"
)
...
func (m *defaultShortUrlMapModel) Delete(ctx context.Context, id int64) error {
	...
	mallShortUrlMapIdKey := fmt.Sprintf("%s%v", cacheMallShortUrlMapIdPrefix, id)
	mallShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheMallShortUrlMapMd5Prefix, data.Md5)
	mallShortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheMallShortUrlMapSurlPrefix, data.Surl)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, mallShortUrlMapIdKey, mallShortUrlMapMd5Key, mallShortUrlMapSurlKey)
	return err
}
...
func (m *defaultShortUrlMapModel) FindOne(ctx context.Context, id int64) (*ShortUrlMap, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, mallShortUrlMapIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultShortUrlMapModel) FindOneByMd5(ctx context.Context, md5 sql.NullString) (*ShortUrlMap, error) {
	mallShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheMallShortUrlMapMd5Prefix, md5)
	var resp ShortUrlMap
	err := m.QueryRowIndexCtx(ctx, &resp, mallShortUrlMapMd5Key, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultShortUrlMapModel) FindOneBySurl(ctx context.Context, surl sql.NullString) (*ShortUrlMap, error) {
	mallShortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheMallShortUrlMapSurlPrefix, surl)
	var resp ShortUrlMap
	err := m.QueryRowIndexCtx(ctx, &resp, mallShortUrlMapSurlKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultShortUrlMapModel) Insert(ctx context.Context, data *ShortUrlMap) (sql.Result, error) {
	mallShortUrlMapIdKey := fmt.Sprintf("%s%v", cacheMallShortUrlMapIdPrefix, data.Id)
	mallShortUrlMapMd5Key := fmt.Sprintf("%s%v", cacheMallShortUrlMapMd5Prefix, data.Md5)
	mallShortUrlMapSurlKey := fmt.Sprintf("%s%v", cacheMallShortUrlMapSurlPrefix, data.Surl)
...
func (m *defaultShortUrlMapModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheMallShortUrlMapIdPrefix, primary)
}
