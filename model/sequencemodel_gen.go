...
var (
	...
	cacheMallSequenceIdPrefix   = "cache:mall:sequence:id:"
	cacheMallSequenceStubPrefix = "cache:mall:sequence:stub:"
)
...
func (m *defaultSequenceModel) Delete(ctx context.Context, id int64) error {
	...
	mallSequenceIdKey := fmt.Sprintf("%s%v", cacheMallSequenceIdPrefix, id)
	mallSequenceStubKey := fmt.Sprintf("%s%v", cacheMallSequenceStubPrefix, data.Stub)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, mallSequenceIdKey, mallSequenceStubKey)
	return err
}
...
func (m *defaultSequenceModel) FindOne(ctx context.Context, id int64) (*Sequence, error) {
	mallSequenceIdKey := fmt.Sprintf("%s%v", cacheMallSequenceIdPrefix, id)
	var resp Sequence
	err := m.QueryRowCtx(ctx, &resp, mallSequenceIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultSequenceModel) FindOneByStub(ctx context.Context, stub string) (*Sequence, error) {
	mallSequenceStubKey := fmt.Sprintf("%s%v", cacheMallSequenceStubPrefix, stub)
	var resp Sequence
	err := m.QueryRowIndexCtx(ctx, &resp, mallSequenceStubKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultSequenceModel) Insert(ctx context.Context, data *Sequence) (sql.Result, error) {
	mallSequenceIdKey := fmt.Sprintf("%s%v", cacheMallSequenceIdPrefix, data.Id)
	mallSequenceStubKey := fmt.Sprintf("%s%v", cacheMallSequenceStubPrefix, data.Stub)
...
func (m *defaultSequenceModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheMallSequenceIdPrefix, primary)
}
